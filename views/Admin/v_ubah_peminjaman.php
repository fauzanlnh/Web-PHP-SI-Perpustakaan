...
<head>
  ...
  <title>SIPAS | Form Ubah Peminjaman Buku</title>
          ...
          <div class="row mb-2">
            ...
            <div class="col-sm-6">
              <ol class="breadcrumb float-sm-right">
                ...
                <li class="breadcrumb-item active">Transaksi / Data Peminjaman / Form Peminjaman</li>
              </ol>
            </div><!-- /.col -->
          </div><!-- /.row -->
        </div><!-- /.container-fluid -->
     ...
     <section class="content">
      ...
      <div class="card card-default">
          ...
          <div class="card-body">
            <div class="row">
              <div class="col-md-6">
              <form role="form" method="post" action="../../proses/P_UbahPeminjaman.php">
                <div class="card-body">
                  <div class="form-group">
                    <?php
                      if(isset($_GET['pesan'])){
                        $Kd_Peminjaman = $_SESSION['Kd_Peminjaman'];
                        $pesan = $_GET['pesan'];
                        if($pesan == "MasihKosong" || $pesan == "KdTidakAda"){
                            echo"<label>Kode Peminjaman</label>
                                <input type='text' class='form-control' id='exampleInputKd' value='$Kd_Peminjaman' name='Kd_Peminjaman' readonly value='$Kd_Peminjaman'>";
                        }
                      }else{
                        include "../../connection/koneksi.php";
                    ...
                    <input type="text" class="form-control" id="exampleInputKd" value="<?php echo $Kd_Peminjaman?>" name="Kd_Peminjaman" readonly>
                    <?php
                      }
                      ?>
                  </div>
                      ...
                      if(isset($_GET['pesan'])){
                        ...
                        if($pesan == "MasihKosong" || $pesan == "KdTidakAda"){
                          if(($_SESSION['Tanggal']) == '1970-01-01'){
                            ...
                            <div class='input-group date'>
                              <input type='date' class='form-control is-invalid datetimepicker-input' name='Tanggal' readonly>
                            </div>";
                          }else{
                            echo"<label>Tanggal Pinjam:</label>
                                <div class='input-group date'>
                                <input type='date' class='form-control datetimepicker-input' name='Tanggal' value='".$_SESSION['Tanggal']."' readonly>
                                </div>";
                          }
                        }
                      }else{
                        $Data = mysqli_query($koneksi, "SELECT NOW() AS 'HariIni'");
                          if($result = mysqli_fetch_array($Data)){
                            ...
                            echo"<label>Tanggal Pinjam:</label>
                              <div class='input-group date'>
                              <input type='date' class='form-control datetimepicker-input' name='Tanggal' value='".$Tgl_Pinjam."' readonly>
                              </div>";
                            }
                      }
                    ?>
                  ...
                  <div class="form-group">
                    <?php
                      if(isset($_GET['pesan'])){
                        ...
                        echo"<label for='exampleInputPassword1'>Kode Anggota</label>
                        <input type='text' class='form-control' id='exampleInputPassword1' placeholder='Masukkan Kode Anggota' name='Kd_Anggota' value='".$Kd_Anggota."'>";
                      }
                    ?>
                  </div>
                  <div class="form-group">
                    <label>Judul Buku - Kode Buku</label>
                    <select class="form-control select2" style="width: 100%;" name="Kd_Buku">
                    <?php
                        include '../connection/koneksi.php';
                        $Data = mysqli_query($koneksi, "SELECT * FROM t_buku WHERE Status = 'TERSEDIA' OR(Status='DIPINJAM' AND Kd_Buku ='$Kd_Buku_Lama') ORDER BY Judul_Buku ASC");
                        while($result2 = mysqli_fetch_array($Data)){
                          if($_GET['Kd_Peminjaman']){
                            if($Kd_Buku_Lama == $result2['Kd_Buku']){
                              $select = "selected";
                            }else{
                              $select = "";
                            }
                          ...
                          if($_GET['pesan']){
                            $Kd_Buku = $_SESSION['Kd_Buku_Baru'];
                            if($Kd_Buku == $result2['Kd_Buku_Baru']){
                              $select = "selected";
                            }else{
                              $select = "";
                            }
                          }
                          echo"<option $select value='".$result2['Kd_Buku']."'>";
                          echo $result2['Judul_Buku'] ." - ". $result2['Kd_Buku'];
                          echo"</option>";
                        }
                      ?>
                    </select>
